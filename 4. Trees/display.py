...
def _display_aux(node):
    ...
    if node.get_right() is None:
        lines, n, p, x = _display_aux(node.get_left())
        s = f'{node.get_data()}'
        u = len(s)
    ...
    if node.get_left() is None:
        lines, n, p, x = _display_aux(node.get_right())
        s = f'{node.get_data()}'
        u = len(s)
    ...
    # Two children.
    left, n, p, x = _display_aux(node.get_left())
    right, m, q, y = _display_aux(node.get_right())
    s = f'{node.get_data()}'
    u = len(s)
